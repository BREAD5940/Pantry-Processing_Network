...
public class MultiplicationValueNode extends ValueNode<Double> {
	...
	/**
	 * Multiplies the value of two nodes together and returns them. The values
	 * are converted to doubles as a side affect.
	 * 
	 * @param network
	 *            This' Network.
	 * @param logger
	 ...
	 */
	public MultiplicationValueNode(Network network, Logger logger, ValueNode<? extends Number> multiple1ValueNode,
			ValueNode<? extends Number> multiple2ValueNode) throws IllegalArgumentException, IllegalStateException {
		super(network, logger, multiple1ValueNode, multiple2ValueNode);
	 ...
	 * are converted to doubles as a side affect.
	 * 
	 * The second multiple is converted to a constant value node and that is
	 * used as the value to multiply.
	 * 
	 * @param network
	 *            This' Network
	 * @param logger
	...
	public MultiplicationValueNode(Network network, Logger logger, ValueNode<? extends Number> multiple1ValueNode,
			double multiple2) {
		this(network, logger, multiple1ValueNode, new ConstantValueNode<>(network, logger, multiple2));
	}
