...
public class ProcessingNetworkUtils {
	 ...
	 * that is passed to the Node class on initialization of a subclass. If any node
	 * passed in is null, the method simply returns a new empty array, leaving the
	 * checking and exception throwing to you (the person writing a constructor).
	 * 
	 * @param node
	 *            A {@link ValueNode} to add to the returned array.
	 * @param nodes
	 *            {@link ValueNode}(s) to add to the returned array.
	 * @return An array of {@link ValueNode}s containing node and the contents of
	 *         nodes unless either of them was/ contained null (in which case an
	 *         empty array).
	 */
	 ...
	 * @param nodes
	 *            {@link ValueNode}(s) to add to the returned array.
	 * @return An array of {@link ValueNode}s containing the contents of nodeArray
	 *         and nodes unless either of them was/ contained null (in which case an
	 *         empty array).
	 */
	public static ValueNode<?>[] concatValueNodes(ValueNode<?>[] nodeArray, ValueNode<?>... nodes) {
		if (nodeArray != null && nodes != null) {
			...
			throw new IllegalArgumentException("Value Node Array Null");
		}
	}
	 ...
	 * This is a static utility method for constructing the array of value nodes
	 * that is passed to the Node class on initialization of a subclass. If any node
	 * passed in is null or the collection is null, the method simply returns a new
	 * empty array, leaving the checking and exception throwing to you (the person
	 * writing a constructor).
	 * 
	 * @param nodes
	 *            A collection of nodes to convert into an array.
	 * @return An array containing the contents of nodes unless it or it's contents
	 *         are null in which case a new empty array.
	 */
	...
	public static ValueNode<?>[] valueNodesMapToArray(Map<? extends Object, ? extends ValueNode<?>> nodes) {
		if (nodes == null) {
			throw new IllegalArgumentException("Node Map Null");
		}
